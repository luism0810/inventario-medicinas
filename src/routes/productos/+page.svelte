<script lang="ts">
  ...
  // Carga dinámica del componente ProductForm solo en el navegador.
  // Esto evita que el servidor intente resolver sus importaciones (como $lib/api/productos).
  onMount(async () => {
  ...
  function onAddProduct() {
    editing = null; // Para indicar que es un nuevo producto
    showForm = true;
  }

  // Cuando el formulario se guarda, en lugar de recargar manualmente,
  // invalidamos los datos para que SvelteKit los vuelva a cargar automáticamente.
  async function onSaved() {
    // await invalidateAll(); // Descomentar si se necesita una recarga forzada de los datos.
    onClose();
    goto('/productos', { replaceState: true, invalidateAll: true }); // Forzamos la recarga para ver los cambios.
  }
...
<div class="flex justify-between items-center mb-6">
  ...
  <button type="button" on:click={onAddProduct} class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-3">
                                <!-- Cambiado: usar botón que abre modal en lugar de enlace -->
                                <button type="button" class="text-indigo-600 hover:text-indigo-900" title="Editar" on:click={() => onEdit(producto)}>
...
{:else}
    <div class="text-center py-12">
        <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" />
        </svg>
